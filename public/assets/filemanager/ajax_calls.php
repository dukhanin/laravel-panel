...
{
	response(rf_trans('forbiden').AddErrorLocation())->send();
	exit;
}
...
if (isset($_SESSION['RF']['language']) && file_exists('lang/' . basename($_SESSION['RF']['language']) . '.php'))
{
	if(array_key_exists($_SESSION['RF']['language'],$languages)){
		...
		response(rf_trans('Lang_Not_Found').AddErrorLocation())->send();
		exit;
	}
} else {
	response(rf_trans('Lang_Not_Found').AddErrorLocation())->send();
...
if(isset($_GET['action']))
{
	...
	{
		case 'new_file_form':
			echo rf_trans('Filename') . ': <input type="text" id="create_text_file_name" style="height:30px"> <select id="create_text_file_extension" style="margin:0;width:100px;">';
		...
		case 'view':
			...
			{
				response(rf_trans('view type number missing').AddErrorLocation())->send();
				exit;
			}
			break;
		case 'filter':
		...
		case 'save_img':
			...
			{
				response(rf_trans('wrong data').AddErrorLocation())->send();
				exit;
			}
			$image_data = get_file_by_url($_POST['url']);
			if ($image_data === false)
			{
				response(rf_trans('Aviary_No_Save').AddErrorLocation())->send();
				exit;
			}

			if (!checkresultingsize(strlen($image_data))) {
				response(sprintf(rf_trans('max_size_reached'),$MaxSizeTotal).AddErrorLocation())->send();
				exit;
			}
			if($ftp){
		...
		case 'extract':
			if (	strpos($_POST['path'], '/') === 0 
				...
				|| strpos($_POST['path'], './') === 0)
			{
				response(rf_trans('wrong path'.AddErrorLocation()))->send();
				exit;
			}
			...
			switch ($info['extension'])
			{
				case "zip":
					...
					if ($zip->open($path) === true)
					{
						...
						response(rf_trans('Zip_No_Extract').AddErrorLocation())->send();
						exit;
					}
				...
				default:
					response(rf_trans('Zip_Invalid').AddErrorLocation())->send();
					exit;
			}
		...
		case 'copy_cut':
			...
			{
				response(rf_trans('wrong sub-action').AddErrorLocation())->send();
				exit;
			}
			...
			if (trim($_POST['path']) == '')
			{
				response(rf_trans('no path').AddErrorLocation())->send();
				exit;
			}

			$msg_sub_action = ($_POST['sub_action'] == 'copy' ? rf_trans('Copy') : rf_trans('Cut'));
			$path = $current_path . $_POST['path'];
			...
			if (is_dir($path))
			{
				...
				{
					response(sprintf(rf_trans('Copy_Cut_Not_Allowed'), $msg_sub_action, rf_trans('Folders')).AddErrorLocation())->send();
					exit;
				}
				...
				if ($copy_cut_max_size !== false && is_int($copy_cut_max_size)) {
					if (($copy_cut_max_size * 1024 * 1024) < $sizeFolderToCopy) {
						response(sprintf(rf_trans('Copy_Cut_Size_Limit'), $msg_sub_action, $copy_cut_max_size).AddErrorLocation())->send();
						exit;
					}
				}
				...
				if ($copy_cut_max_count !== false && is_int($copy_cut_max_count))
				{
					...
					{
						response(sprintf(rf_trans('Copy_Cut_Count_Limit'), $msg_sub_action, $copy_cut_max_count).AddErrorLocation())->send();
						exit;
					}
				}
				...
				{
					response(sprintf(rf_trans('Copy_Cut_Not_Allowed'), $msg_sub_action, rf_trans('Files')).AddErrorLocation())->send();
					exit;
				}
			}
		...
		case 'chmod':
			if($ftp){
				...
				{
					response(sprintf(rf_trans('File_Permission_Not_Allowed'), (is_dir($path) ? rf_trans('Folders') : rf_trans('Files')), 403).AddErrorLocation())->send();
					exit;
				}
			...
			if ((!$ftp && is_dir($path)) )
			{
				$ret .= '<div class="hero-unit" style="padding:10px;">'.rf_trans('File_Permission_Recursive').'<br/><br/>
						<ul class="unstyled">
							<li><label class="radio"><input value="none" name="apply_recursive" type="radio" checked> '.rf_trans('No').'</label></li>
							<li><label class="radio"><input value="files" name="apply_recursive" type="radio"> '.rf_trans('Files').'</label></li>
							<li><label class="radio"><input value="folders" name="apply_recursive" type="radio"> '.rf_trans('Folders').'</label></li>
							<li><label class="radio"><input value="both" name="apply_recursive" type="radio"> '.rf_trans('Files').' & '.rf_trans('Folders').'</label></li>
						</ul>
						</div>';
			}
		...
		case 'get_lang':
			...
			break;
		case 'change_lang':
			$choosen_lang = (!empty($_POST['choosen_lang']))? $_POST['choosen_lang']:"en";
		...
		case 'cad_preview':
			if($ftp){
				...
				{
					response(rf_trans('File_Not_Found').AddErrorLocation())->send();
					exit;
				}
			}
			if($ftp){
		...
		case 'get_file': // preview or edit
			...
			{
				response(rf_trans('wrong action').AddErrorLocation())->send();
				exit;
			}
			...
			{
				response(sprintf(rf_trans('File_Open_Edit_Not_Allowed'), ($sub_action == 'preview' ? strtolower(rf_trans('Open')) : strtolower(rf_trans('Edit')))).AddErrorLocation())->send();
				exit;
			}
			...
			break;
		default:
			response(rf_trans('no action passed').AddErrorLocation())->send();
			exit;
	}
} else {
	response(rf_trans('no action passed').AddErrorLocation())->send();
