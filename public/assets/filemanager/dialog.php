...
<html xmlns="https://www.w3.org/1999/xhtml">
	...
	<script>
		...
		Dropzone.options.rfmDropzone = {
			dictInvalidFileType: "<?php echo rf_trans('Error_extension');?>",
			dictFileTooBig: "<?php echo rf_trans('Error_Upload');?>",
			dictDefaultMessage: "<?php echo rf_trans('Upload_message');?>",
			dictResponseError: "<?php echo rf_trans('SERVER ERROR');?>",
			...
			accept: function(file, done) {
				...
				if ($.inArray(extension, allowed_ext) > -1) {
					...
					done("<?php echo rf_trans('Error_extension');?>");
				}
			}
		};
		if (image_editor) {
...
<body>
	...
	<input type="hidden" id="insert_folder_name" value="<?php echo rf_trans('Insert_Folder_Name');?>" />
	<input type="hidden" id="new_folder" value="<?php echo rf_trans('New_Folder');?>" />
	<input type="hidden" id="ok" value="<?php echo rf_trans('OK');?>" />
	<input type="hidden" id="cancel" value="<?php echo rf_trans('Cancel');?>" />
	<input type="hidden" id="rename" value="<?php echo rf_trans('Rename');?>" />
	<input type="hidden" id="lang_duplicate" value="<?php echo rf_trans('Duplicate');?>" />
	...
	<input type="hidden" id="lang_show_url" value="<?php echo rf_trans('Show_url');?>" />
	...
	<input type="hidden" id="lang_copy" value="<?php echo rf_trans('Copy');?>" />
	<input type="hidden" id="lang_cut" value="<?php echo rf_trans('Cut');?>" />
	<input type="hidden" id="lang_paste" value="<?php echo rf_trans('Paste');?>" />
	<input type="hidden" id="lang_paste_here" value="<?php echo rf_trans('Paste_Here');?>" />
	<input type="hidden" id="lang_paste_confirm" value="<?php echo rf_trans('Paste_Confirm');?>" />
	<input type="hidden" id="lang_files" value="<?php echo rf_trans('Files');?>" />
	<input type="hidden" id="lang_folders" value="<?php echo rf_trans('Folders');?>" />
	<input type="hidden" id="lang_files_on_clipboard" value="<?php echo rf_trans('Files_ON_Clipboard');?>" />
	<input type="hidden" id="clipboard" value="<?php echo ((isset($_SESSION['RF']['clipboard']['path']) && trim($_SESSION['RF']['clipboard']['path']) != null) ? 1 : 0);?>" />
	<input type="hidden" id="lang_clear_clipboard_confirm" value="<?php echo rf_trans('Clear_Clipboard_Confirm');?>" />
	<input type="hidden" id="lang_file_permission" value="<?php echo rf_trans('File_Permission');?>" />
	...
	<input type="hidden" id="lang_lang_change" value="<?php echo rf_trans('Lang_Change');?>" />
	<input type="hidden" id="edit_text_files_allowed" value="<?php if($edit_text_files) echo 1; else echo 0;?>" />
	<input type="hidden" id="lang_edit_file" value="<?php echo rf_trans('Edit_File');?>" />
	<input type="hidden" id="lang_new_file" value="<?php echo rf_trans('New_File');?>" />
	<input type="hidden" id="lang_filename" value="<?php echo rf_trans('Filename');?>" />
	<input type="hidden" id="lang_file_info" value="<?php echo fix_strtoupper(rf_trans('File_info'));?>" />
	<input type="hidden" id="lang_edit_image" value="<?php echo rf_trans('Edit_image');?>" />
	<input type="hidden" id="lang_error_upload" value="<?php echo rf_trans('Error_Upload');?>" />
	<input type="hidden" id="lang_select" value="<?php echo rf_trans('Select');?>" />
	<input type="hidden" id="lang_extract" value="<?php echo rf_trans('Extract');?>" />
...
<div class="uploader">
	<div class="text-center">
		<button class="btn btn-inverse close-uploader"><i class="icon-backward icon-white"></i> <?php echo rf_trans('Return_Files_List')?></button>
	</div>
	<div class="space10"></div>
	<div class="space10"></div>
	<div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
		<ul class="nav nav-tabs">
			<li class="active"><a href="#tab1" data-toggle="tab"><?php echo rf_trans('Upload_base');?></a></li>
			<?php if($url_upload){ ?>
			<li><a href="#taburl" data-toggle="tab"><?php echo rf_trans('Upload_url');?></a></li>
			<?php } ?>
			<?php if($java_upload){ ?>
			<li><a href="#tab2" id="uploader-btn" data-toggle="tab"><?php echo rf_trans('Upload_java');?></a></li>
			<?php } ?>
		</ul>
		<div class="tab-content">
			<div class="tab-pane active" id="tab1">
				<form action="dialog.php" method="post" enctype="multipart/form-data" id="rfmDropzone" class="dropzone">
					...
					<div class="fallback">
						<h3><?php echo  rf_trans('Upload_file')?>:</h3><br/>
						...
						<input type="submit" name="submit" value="<?php echo rf_trans('OK')?>" />
					</div>
				</form>
				<div class="upload-help"><?php echo rf_trans('Upload_base_help');?></div>
			</div>
			<?php if($url_upload){ ?>
			<div class="tab-pane" id="taburl">
				...
				<form class="form-horizontal">
					<div class="control-group">
						<label class="control-label" for="url"><?php echo rf_trans('Upload_url');?></label>
						<div class="controls">
							<input type="text" class="input-block-level" id="url" placeholder="<?php echo rf_trans('Upload_url');?>">
						</div>
					</div>
					<div class="control-group">
						<div class="controls">
							<button class="btn btn-primary" id="uploadURL"><?php echo  rf_trans('Upload_file');?></button>
						</div>
					</div>
				</form>
			</div>
			...
			<div class="tab-pane" id="tab2">
				...
				<div class="upload-help"><?php echo rf_trans('Upload_java_help');?></div>
			</div>
			<?php } ?>
		</div>
	</div>
...
<div class="navbar navbar-fixed-top">
	<div class="navbar-inner">
		...
		<div class="brand"><?php echo rf_trans('Toolbar');?></div>
		<div class="nav-collapse collapse">
		<div class="filters">
			<div class="row-fluid">
			<div class="span4 half">
				<?php if($upload_files){ ?>
				<button class="tip btn upload-btn" title="<?php echo  rf_trans('Upload_file');?>"><i class="rficon-upload"></i></button>
				<?php } ?>
				<?php if($create_text_files){ ?>
				<button class="tip btn create-file-btn" title="<?php echo  rf_trans('New_File');?>"><i class="icon-plus"></i><i class="icon-file"></i></button>
				<?php } ?>
				<?php if($create_folders){ ?>
				<button class="tip btn new-folder" title="<?php echo  rf_trans('New_Folder')?>"><i class="icon-plus"></i><i class="icon-folder-open"></i></button>
				<?php } ?>
				<?php if($copy_cut_files || $copy_cut_dirs){ ?>
				<button class="tip btn paste-here-btn" title="<?php echo rf_trans('Paste_Here');?>"><i class="rficon-clipboard-apply"></i></button>
				<button class="tip btn clear-clipboard-btn" title="<?php echo rf_trans('Clear_Clipboard');?>"><i class="rficon-clipboard-clear"></i></button>
				<?php } ?>
			</div>
			<div class="span2 half view-controller">
				<button class="btn tip<?php if($view==0) echo " btn-inverse";?>" id="view0" data-value="0" title="<?php echo rf_trans('View_boxes');?>"><i class="icon-th <?php if($view==0) echo "icon-white";?>"></i></button>
				<button class="btn tip<?php if($view==1) echo " btn-inverse";?>" id="view1" data-value="1" title="<?php echo rf_trans('View_list');?>"><i class="icon-align-justify <?php if($view==1) echo "icon-white";?>"></i></button>
				<button class="btn tip<?php if($view==2) echo " btn-inverse";?>" id="view2" data-value="2" title="<?php echo rf_trans('View_columns_list');?>"><i class="icon-fire <?php if($view==2) echo "icon-white";?>"></i></button>
			</div>
			<div class="span6 entire types">
				<span><?php echo rf_trans('Filters');?>:</span>
				<?php if($_GET['type']!=1 && $_GET['type']!=3 && $show_filter_buttons){ ?>
					<?php if(count($ext_file)>0 or false){ ?>
				<input id="select-type-1" name="radio-sort" type="radio" data-item="ff-item-type-1" checked="checked"  class="hide"  />
				<label id="ff-item-type-1" title="<?php echo rf_trans('Files');?>" for="select-type-1" class="tip btn ff-label-type-1"><i class="icon-file"></i></label>
					<?php } ?>
					<?php if(count($ext_img)>0 or false){ ?>
				<input id="select-type-2" name="radio-sort" type="radio" data-item="ff-item-type-2" class="hide"  />
				<label id="ff-item-type-2" title="<?php echo rf_trans('Images');?>" for="select-type-2" class="tip btn ff-label-type-2"><i class="icon-picture"></i></label>
					<?php } ?>
					<?php if(count($ext_misc)>0 or false){ ?>
				<input id="select-type-3" name="radio-sort" type="radio" data-item="ff-item-type-3" class="hide"  />
				<label id="ff-item-type-3" title="<?php echo rf_trans('Archives');?>" for="select-type-3" class="tip btn ff-label-type-3"><i class="icon-inbox"></i></label>
					<?php } ?>
					<?php if(count($ext_video)>0 or false){ ?>
				<input id="select-type-4" name="radio-sort" type="radio" data-item="ff-item-type-4" class="hide"  />
				<label id="ff-item-type-4" title="<?php echo rf_trans('Videos');?>" for="select-type-4" class="tip btn ff-label-type-4"><i class="icon-film"></i></label>
					<?php } ?>
					<?php if(count($ext_music)>0 or false){ ?>
				<input id="select-type-5" name="radio-sort" type="radio" data-item="ff-item-type-5" class="hide"  />
				<label id="ff-item-type-5" title="<?php echo rf_trans('Music');?>" for="select-type-5" class="tip btn ff-label-type-5"><i class="icon-music"></i></label>
					<?php } ?>
				<?php } ?>
				<input accesskey="f" type="text" class="filter-input <?php echo (($_GET['type']!=1 && $_GET['type']!=3) ? '' : 'filter-input-notype');?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(rf_trans('Text_filter'));?>..." value="<?php echo $filter;?>"/><?php if($n_files>$file_number_limit_js){ ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>

				<input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
				<label id="ff-item-type-all" title="<?php echo rf_trans('All');?>" <?php if($_GET['type']==1 || $_GET['type']==3){ ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-remove icon-white"></i></label>
...
<!-- header div end -->
	...
	<li class="pull-right">
		...
		<ul class="dropdown-menu pull-left sorting">
			<li class="text-center"><strong><?php echo rf_trans('Sorting') ?></strong></li>
		<li><a class="sorter sort-name <?php if($sort_by=="name"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="name"><?php echo rf_trans('Filename');?></a></li>
		<li><a class="sorter sort-date <?php if($sort_by=="date"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="date"><?php echo rf_trans('Date');?></a></li>
		<li><a class="sorter sort-size <?php if($sort_by=="size"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="size"><?php echo rf_trans('Size');?></a></li>
		<li><a class="sorter sort-extension <?php if($sort_by=="extension"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="extension"><?php echo rf_trans('Type');?></a></li>
		</ul>
		</div>
	</li>
	<li><small class="hidden-phone">(<span id="files_number"><?php echo $current_files_number."</span> ".rf_trans('Files')." - <span id='folders_number'>".$current_folders_number."</span> ".rf_trans('Folders');?>)</small></li>
	<?php if($show_total_size){ ?>
	<li><small class="hidden-phone"><span title="<?php echo rf_trans('total size').$MaxSizeTotal;?>"><?php echo rf_trans('total size').": ".makeSize($sizeCurrentFolder).(($MaxSizeTotal !== false && is_int($MaxSizeTotal))? '/'.$MaxSizeTotal.' '.rf_trans('MB'):'');?></span></small>
	</li>
	<?php } ?>
	</ul>
	</div>
	...
	<div class="span12">
		...
		<?php }else{ ?>
		<h4 id="help"><?php echo rf_trans('Swipe_help');?></h4>
		...
		<div class="file-name"><a class="sorter sort-name <?php if($sort_by=="name"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="name"><?php echo rf_trans('Filename');?></a></div>
		<div class="file-date"><a class="sorter sort-date <?php if($sort_by=="date"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="date"><?php echo rf_trans('Date');?></a></div>
		<div class="file-size"><a class="sorter sort-size <?php if($sort_by=="size"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="size"><?php echo rf_trans('Size');?></a></div>
		<div class='img-dimension'><?php echo rf_trans('Dimension');?></div>
		<div class='file-extension'><a class="sorter sort-extension <?php if($sort_by=="extension"){ echo ($descending)?"descending":"ascending"; } ?>" href="javascript:void('')" data-sort="extension"><?php echo rf_trans('Type');?></a></div>
		<div class='file-operations'><?php echo rf_trans('Operations');?></div>
		</div>
		<?php } ?>
		...
		foreach ($files as $file_array) {
			...
			if($file == '.' || ( substr($file, 0, 1) == '.' && isset( $file_array[ 'extension' ] ) && $file_array[ 'extension' ] == strtolower(rf_trans( 'Type_dir' ) )) || (isset($file_array['extension']) && $file_array['extension']!=strtolower(rf_trans('Type_dir'))) || ($file == '..' && $subdir == '') || in_array($file, $hidden_folders) || ($filter!='' && $n_files>$file_number_limit_js && $file!=".." && stripos($file,$filter)===false)){
				continue;
			}
			...
			<?php }else{ ?>
					...
					<input type="hidden" class="extension" value="<?php echo rf_trans('Type_dir');?>"/>
					<div class="file-date"><?php echo date(rf_trans('Date_type'),$file_array['date']);?></div>
					...
					<?php } ?>
					<div class='file-extension'><?php echo rf_trans('Type_dir');?></div>
					<figcaption>
						<a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($rename_folders && !$file_prevent_rename) echo "rename-folder";?>" title="<?php echo rf_trans('Rename')?>" data-folder="1" data-permissions="<?php echo $file_array['permissions']; ?>" data-path="<?php echo $rfm_subfolder.$subdir.$file;?>">
						<i class="icon-pencil <?php if(!$rename_folders || $file_prevent_rename) echo 'icon-white';?>"></i></a>
						<a href="javascript:void('')" class="tip-left erase-button <?php if($delete_folders && !$file_prevent_delete) echo "delete-folder";?>" title="<?php echo rf_trans('Erase')?>" data-confirm="<?php echo rf_trans('Confirm_Folder_del');?>" data-path="<?php echo $rfm_subfolder.$subdir.$file;?>" >
			...
			foreach ($files as $nu=>$file_array) {
				...
				if($file == '.' || $file == '..' || $file_array['extension']==rf_trans('Type_dir') || in_array($file, $hidden_files) || !in_array(fix_strtolower($file_array['extension']), $ext) || ($filter!='' && $n_files>$file_number_limit_js && stripos($file,$filter)===false))
					continue;
...
?>
			...
			?>		<figure data-name="<?php echo $file ?>" data-type="<?php if($is_img){ echo "img"; }else{ echo "file"; } ?>">
				...
				<figcaption>
					...
					<a title="<?php echo rf_trans('Download')?>" class="tip-right" href="javascript:void('')" onclick="$('#form<?php echo $nu;?>').submit();"><i class="icon-download"></i></a>
					<?php if($is_img && $src_thumb!="" && $file_array['extension']!="tiff" && $file_array['extension']!="tif"){ ?>
					<a class="tip-right preview" title="<?php echo rf_trans('Preview')?>" data-url="<?php echo $src;?>" data-toggle="lightbox" href="#previewLightbox"><i class=" icon-eye-open"></i></a>
					...
					title="<?php echo rf_trans('Preview')?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					...
					<a class="tip-right file-preview-btn" title="<?php echo rf_trans('Preview')?>" data-url="ajax_calls.php?action=cad_preview&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					<?php }elseif($preview_text_files && in_array($file_array['extension'],$previewable_text_file_exts)){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo rf_trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=text&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					href="javascript:void('');" ><i class=" icon-eye-open"></i></a>
					<?php }elseif($googledoc_enabled && in_array($file_array['extension'],$googledoc_file_exts)){ ?>
					<a class="tip-right file-preview-btn" title="<?php echo rf_trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=google&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					...
					<a class="tip-right file-preview-btn" title="<?php echo rf_trans('Preview')?>" data-url="ajax_calls.php?action=get_file&sub_action=preview&preview_mode=viewerjs&title=<?php echo $filename;?>&file=<?php echo $rfm_subfolder.$subdir.$file;?>"
					href="docs.google.com;" ><i class=" icon-eye-open"></i></a>

					<?php }else{ ?>
					<a class="preview disabled"><i class="icon-eye-open icon-white"></i></a>
					<?php } ?>
					<a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php if($rename_files && !$file_prevent_rename) echo "rename-file";?>" title="<?php echo rf_trans('Rename')?>" data-folder="0" data-permissions="<?php echo $file_array['permissions']; ?>" data-path="<?php echo $rfm_subfolder.$subdir .$file;?>">
					<i class="icon-pencil <?php if(!$rename_files || $file_prevent_rename) echo 'icon-white';?>"></i></a>

					<a href="javascript:void('')" class="tip-left erase-button <?php if($delete_files && !$file_prevent_delete) echo "delete-file";?>" title="<?php echo rf_trans('Erase')?>" data-confirm="<?php echo rf_trans('Confirm_del');?>" data-path="<?php echo $rfm_subfolder.$subdir.$file;?>">
...
<script>
	...
	<div class="modal-header">
		...
		<h3><?php echo rf_trans('Preview');?></h3>
	</div>
