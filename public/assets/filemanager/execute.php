...
if (isset($_POST['name']))
{
	...
	{
		response(rf_trans('wrong name').AddErrorLocation())->send();
		exit;
	}
}
...
{
	response(rf_trans('wrong extension').AddErrorLocation())->send();
	exit;
}
	...
	switch($_GET['action'])
	{
		...
		case 'rename_folder':
			if ($rename_folders){
				...
				if (!empty($name)){
					...
					{
						response(rf_trans('Rename_existing_folder').AddErrorLocation())->send();
						exit;
					}
					...
					response(rf_trans('Empty_name').AddErrorLocation())->send();
					exit;
				}
			}
			break;
		case 'create_file':
			if ($create_text_files === FALSE) {
				response(sprintf(rf_trans('File_Open_Edit_Not_Allowed'), strtolower(rf_trans('Edit'))).AddErrorLocation())->send();
				exit;
			}
			...
			if (strpos($name, '.') === FALSE){
				response(rf_trans('No_Extension').' '.sprintf(rf_trans('Valid_Extensions'), implode(', ', $editable_text_file_exts)).AddErrorLocation())->send();
				exit;
			}
			...
			if (!in_array(end($parts), $editable_text_file_exts)) {
				response(rf_trans('Error_extension').' '.sprintf(rf_trans('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
				exit;
			}
			...
			if($ftp){
				...
				unlink($tmp);
				response(rf_trans('File_Save_OK'))->send();
			}else{
				...
				if (file_exists($path.$name)) {
					response(rf_trans('Rename_existing_file').AddErrorLocation())->send();
					exit;
				}

				if (@file_put_contents($path.$name, $content) === FALSE) {
					response(rf_trans('File_Save_Error').AddErrorLocation())->send();
					exit;
				} else {
		...
		case 'paste_clipboard':
			...
			if($ftp){
				...
				if (is_really_writable($path) === FALSE || is_really_writable($path_thumb) === FALSE){
					response(rf_trans('Dir_No_Write').'<br/>'.str_replace('../','',$path).'<br/>'.str_replace('../','',$path_thumb).AddErrorLocation())->send();
					exit;
				}

				// check if server disables copy or rename
				if (is_function_callable(($action == 'copy' ? 'copy' : 'rename')) === FALSE){
					response(sprintf(rf_trans('Function_Disabled'), ($action == 'copy' ? (rf_trans('Copy')) : (rf_trans('Cut')))).AddErrorLocation())->send();
					exit;
				}
				if ($action == 'copy')
				{
		...
		case 'chmod':
			...
			// check perm
			if ($chmod_perm === FALSE) {
				response(sprintf(rf_trans('File_Permission_Not_Allowed'), (is_dir($path) ? (rf_trans('Folders')) : (rf_trans('Files')) )).AddErrorLocation())->send();
				exit;
			}
			// check mode
			if (!preg_match("/^[0-7]{3}$/", $mode)){
				response(rf_trans('File_Permission_Wrong_Mode').AddErrorLocation())->send();
				exit;
			}
			// check recursive option
			if (!in_array($rec_option, $valid_options)){
				response(rf_trans("wrong option").AddErrorLocation())->send();
				exit;
			}
			// check if server disabled chmod
			if (!$ftp && is_function_callable('chmod') === FALSE){
				response(sprintf(rf_trans('Function_Disabled'), 'chmod').AddErrorLocation())->send();
				exit;
			}
