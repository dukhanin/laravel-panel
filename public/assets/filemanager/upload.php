...
{
	response(rf_trans('forbiden').AddErrorLocation(), 403)->send();
	exit;
}
...
if ( ! empty($_FILES) || isset($_POST['url']))
{
	...
	if (in_array(fix_strtolower($extension), $ext))
	{
		...
		if (!checkresultingsize($_FILES['file']['size'])) {
			response(sprintf(rf_trans('max_size_reached'),$MaxSizeTotal).AddErrorLocation(), 406)->send();
			exit;
		}
		...
		if ($is_img)
		{
			...
			{
				unlink($targetFile);
				response(rf_trans("Not enought Memory").AddErrorLocation(), 406)->send();
				exit();
			}
		}
	...
	{
		response(rf_trans("Error_extension").AddErrorLocation(), 406)->send();
...
{
	response(rf_trans("no file").AddErrorLocation(), 405)->send();
	exit();
}
